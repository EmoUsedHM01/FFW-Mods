...
const rawOutputRoot = path.join(root, "RawChunks", "raw_mod_branch_target");
const rawOutputDir = path.join(rawOutputRoot, "chunks");
const jsonOutputDir = path.join(scriptUtilsDir, "patched_branch_target");
const stageRoot = path.join(root, "ModStageBranchTarget");
const pakListPath = path.join(scriptUtilsDir, "WeakspotEveryHit_BranchTarget_PakList.txt");
...
const jumpTargetRaw = [
  {
    id: "2ee165222bd1131e00000001",
    name: "BP_PlayerBullet.F_ApplyDamages hitCriticalBones false target",
    opcodeOffset: 0xb84b,
    argOffset: 0xb84c,
    expectedOpcode: 0x07,
    expectedArg: 0xfce,
    replacementArg: 0x14dc,
  },
  {
    id: "c888831310d62c0a00000001",
    name: "BP_ArrowProjectile.F_IsCritical hitCriticalBones false target",
    opcodeOffset: 0x8211,
    argOffset: 0x8212,
    expectedOpcode: 0x07,
    expectedArg: 0x243,
    replacementArg: 0x39f,
  },
];

const jumpTargetJson = [
  {
    file: "BP_PlayerBullet_35.json",
    exportName: "F_ApplyDamages",
    name: "BP_PlayerBullet.F_ApplyDamages hitCriticalBones false target",
...
function patchJumpTarget(buffer, patch) {
  ...
  if (arg !== patch.expectedArg) {
    throw new Error(`${patch.name}: expected jump target 0x${patch.expectedArg.toString(16)} at 0x${patch.argOffset.toString(16)}, found 0x${arg.toString(16)}`);
  }
...
const rawIds = new Set([
  ...
  ...jumpTargetRaw.map((patch) => patch.id),
]);
...
for (const id of rawIds) {
  ...
  for (const patch of jumpTargetRaw.filter((entry) => entry.id === id)) {
    patchJumpTarget(data, patch);
    console.log(`${patch.name}: raw jump target 0x${patch.expectedArg.toString(16)}->0x${patch.replacementArg.toString(16)} at 0x${patch.argOffset.toString(16)}`);
  }
...
const jsonFiles = new Set([
  ...
  ...jumpTargetJson.map((patch) => patch.file),
]);
...
for (const file of jsonFiles) {
  ...
  for (const patch of jumpTargetJson.filter((entry) => entry.file === file)) {
    const exportEntry = getExportData(asset, file, patch.exportName);
    const data = Buffer.from(exportEntry.Data, "base64");
    patchJumpTarget(data, patch);
    exportEntry.Data = data.toString("base64");
    console.log(`${patch.name}: json jump target 0x${patch.expectedArg.toString(16)}->0x${patch.replacementArg.toString(16)} at 0x${patch.argOffset.toString(16)}`);
  }
