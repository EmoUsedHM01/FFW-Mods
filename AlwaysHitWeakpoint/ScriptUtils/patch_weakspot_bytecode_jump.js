...
function patchCall(exportData, call) {
  const jumpTarget = call.end + 1;
  exportData[call.start] = 0x27; // EX_True
  exportData[call.start + 1] = 0x06; // EX_Jump
  exportData.writeInt32LE(jumpTarget, call.start + 2);
  exportData.fill(0x0b, call.start + 6, call.end + 1); // EX_Nothing padding
}

const outputRoot = path.join(root, "patched_jump");
fs.rmSync(outputRoot, { recursive: true, force: true });
fs.mkdirSync(outputRoot, { recursive: true });

for (const mod of mods) {
  const input = path.join(root, mod.file);
  const output = path.join(outputRoot, mod.file);
